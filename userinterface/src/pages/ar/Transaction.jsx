...
import logogo from '../../assets/images/logogo.jpeg'
        ...
        <div className="min-h-screen bg-[#f8f7f5] py-10 px-5 md:py-12 md:px-8 lg:py-5 lg:px-0 rtl">
            ...
            */}
            ...
            <div className="print-area">
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', fontFamily: 'Calibri, Arial, sans-serif' }}>
                    <thead>
                        {/* Line 1 — logo only */}
                        <tr>
                            <th colSpan={11} style={{ padding: '10px 6px 4px', border: '1px solid #000', borderBottom: 'none', background: '#FFFFFF' }}>
                                <div style={{ display: 'flex', justifyContent: 'center' }}>
                                    <img src={logogo} alt="logo" style={{ height: 64, width: 64, objectFit: 'cover', borderRadius: '50%' }} />
                                </div>
                            </th>
                        </tr>
                    ...
                    <tbody>
                        {printRows.map((t, index) => (
                            <tr key={t.id} style={{ background: index % 2 === 0 ? '#FFFFFF' : '#FAF7F0' }}>
                                ...
                                <td style={tdStyle}>{t.account_from || '-'}</td>
...
//     // Print state — the printable worksheet is built from the FULL filtered dataset,
//     // not just the current page, so the report matches the .xlsx export.
//     const [printData, setPrintData] = useState([]);
...
//     // Build the query string shared by the paginated fetch and the "fetch everything for print" call
//     const buildQuery = ({ page, size }) => {
//         let url = `${BASE}/api/transactions/?page=${page}&page_size=${size}`;
//         if (searchTerm) url += `&search=${encodeURIComponent(searchTerm)}`;
//         if (filterType) url += `&type=${filterType}`;
...
//     // Once print data has been prepared, trigger the browser print dialog
//     useEffect(() => {
//         if (!isPreparingPrint && printData.length > 0) {
...
//     // Fetch the ENTIRE filtered result set (not just this page) and hand it to the print view
...
//             toast.error('❌ تعذر تجهيز نسخة الطباعة');
//             setIsPreparingPrint(false);
//         }
//     };
...
//     // Totals for the current page — mirrors the table footer below
//     const pageStats = useMemo(() => {
//         const totalDeposit = transactions.reduce(
//             (sum, t) => (t.type === 'deposit' ? sum + parseFloat(t.amount || 0) : sum),
//             0
...
//     // Totals + running balance for the full print dataset
//     const printRows = useMemo(() => {
//         let running = 0;
//         return printData.map((t) => {
//             const amount = parseFloat(t.amount || 0);
...
//             {/* Header */}
//             <div className="no-print flex flex-col shadow-lg sm:flex-row justify-between items-center max-w-full mx-auto px-4 py-2 md:px-3 mb-8 md:mb-10 lg:mb-12 gap-4 bg-white rounded-lg">
//                 <div className="flex items-center gap-4 text-center sm:text-right">
//                     <img src={logogo} alt="Broker City Properties" className="hidden sm:block h-14 w-14 rounded-full object-cover shadow" />
//                     <div>
...
//                     <button
//                         className="flex items-center justify-center gap-2 bg-blue-600 cursor-pointer text-white px-6 md:px-8 py-3 rounded-sm font-extrabold text-sm md:text-base uppercase tracking-wide transition-all duration-300 hover:bg-[#a47b00] hover:text-white hover:scale-105 hover:shadow-lg active:scale-95 whitespace-nowrap disabled:opacity-60 disabled:cursor-not-allowed disabled:hover:scale-100"
//                         onClick={handlePrint}
//                         disabled={isPreparingPrint}
//                     >
//                         {isPreparingPrint ? 'جارٍ التجهيز...' : 'طباعة'}
...
//                                     {/* Table Footer with Totals (current page) */}
...
//                                             </td>
//                                             <td className={`px-6 py-4 text-right text-sm font-bold ${pageStats.balance >= 0 ? 'text-green-600' : 'text-red-600'}`}>
//                                                 {formatMoney(pageStats.balance)}
//                                             </td>
//                                             <td colSpan="2"></td>
//                                         </tr>
...
//             {/* ================= PRINT VIEW — styled to match the Accounting Worksheet .xlsx ================= */}
//             <div className="print-area">
//                 {/* Title bar — white background, bold black text, matches xlsx row 1 */}
//                 <table style={{ width: '100%', borderCollapse: 'collapse', fontFamily: 'Calibri, Arial, sans-serif' }}>
//                     <thead>
...
//                             <th colSpan={11} style={{ padding: '10px 6px 4px', border: '1px solid #000', borderBottom: 'none', background: '#FFFFFF' }}>
//                                 <div style={{ display: 'flex', justifyContent: 'center' }}>
//                                     <img src={logogo} alt="logo" style={{ height: 120, width: 120, objectFit: 'cover', borderRadius: '50%' }} />
//                                 </div>
//                             </th>
//                         </tr>
//                         {/* Line 2 — headers/title text, underneath the logo */}
...
//                                 Gross Total — المجموع الكلي
//                             </td>
//                             <td style={{ ...tdStyle, fontWeight: 800 }}>{formatMoney(printTotals.totalIncome)}</td>
//                             <td style={{ ...tdStyle, fontWeight: 800 }}>{formatMoney(printTotals.totalExpense)}</td>
//                             <td style={{ ...tdStyle, fontWeight: 800 }}>{formatMoney(printTotals.balance)}</td>
//                         </tr>
//                     </tfoot>
//                 </table>
...
//                     <button
//                         className="bg-blue-600 cursor-pointer text-white px-6 md:px-8 py-3 rounded-sm font-extrabold text-sm md:text-base uppercase tracking-wide transition-all duration-300 hover:bg-[#8a6a44] hover:text-white hover:scale-105 hover:shadow-lg active:scale-95 whitespace-nowrap"
//                         onClick={() => {
//                             setSelectedTransaction(null);
//                             setShowAddWithdrawModal(true);
//                         }}
