...
const EditRental = ({
    ...
    // ---------------------------------------------------------
    // Fetch rental data (if not passed in as prop)
    // ---------------------------------------------------------
    useEffect(() => {
        let cancelled = false;

        const loadRental = async () => {
            // If a full rental object was passed, use it directly
            if (passedRental) {
                if (!cancelled) {
                    setFormData({
                        name: passedRental.name ?? "",
                        nationality:
                            passedRental.nationality ?? "",
                        phone_1: passedRental.phone_1 ?? "",
                        phone_2: passedRental.phone_2 ?? "",
                        id_number:
                            passedRental.id_number ?? "",
                        email: passedRental.email ?? "",
                        insurance_balance:
                            passedRental.insurance_balance ??
                            "",
                        balance: passedRental.balance ?? "",
                    });
    ...
    // Focus name when ready
    useEffect(() => {
        if (!loadingData && nameRef.current) {
    ...
    const isNationalityFilled =
        String(formData.nationality || "").trim() !== "";
    const isPhone1Filled =
        String(formData.phone_1 || "").trim() !== "";
    const isPhone2Filled =
        String(formData.phone_2 || "").trim() !== "";
    const isIdNumberFilled =
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const requestData = {
                ...
                nationality:
                    String(formData.nationality).trim() || null,
                phone_1:
                    String(formData.phone_1).trim() || null,
                phone_2:
                    String(formData.phone_2).trim() || null,
                id_number:
                    String(formData.id_number).trim() || null,
                email:
                    String(formData.email).trim() || null,
                insurance_balance:
                    formData.insurance_balance === ""
                        ? 0
                        : Number(formData.insurance_balance),
            ...
            const response = await fetch(
                `${BASE}/api/rentals/${rentalId}/update/`,
                {
                    method: "PUT",
                    headers: {
